        ...
        for (const nikaya of nikayas) {
            ...
            if (nikaya === 'dn' || nikaya === 'mn') {
                ...
                if (dataToBeWritten.length) {
                    ...
                    await promises_1.default.writeFile(outputFile, JSON.stringify(dataToBeWritten, null, 2), // pretty print
                    'utf8');
                }
            }
            if (nikaya === 'sn' || nikaya === 'an') {
                ...
                for (let subNikayaNumber = 1; subNikayaNumber <= numberOfSubNikayas; subNikayaNumber++) {
                    const dataToBeWritten = await extractSuttaCardMetadataFromFolder(path_1.default.join(nikayaPath, String(subNikayaNumber)), nikaya);
                    if (dataToBeWritten.length) {
                        const outPutPath = nikaya === 'sn' ? 'public/_data/suttaCardData/sn' : 'public/_data/suttaCardData/an';
                        await promises_1.default.writeFile(path_1.default.join(process.cwd(), outPutPath, `/${subNikayaNumber}.json`), JSON.stringify(dataToBeWritten, null, 2), // pretty print
                        'utf8');
                    }
                }
            }
            if (nikaya === 'kn') {
                ...
                for (const subNikaya of subNikayas) {
                    const dataToBeWritten = await extractSuttaCardMetadataFromFolder(path_1.default.join(nikayaPath, String(subNikaya)), nikaya);
                    if (dataToBeWritten.length) {
                        const outPutPath = 'public/_data/suttaCardData/kn';
                        await promises_1.default.writeFile(path_1.default.join(process.cwd(), outPutPath, `/${String(subNikaya)}.json`), JSON.stringify(dataToBeWritten, null, 2), // pretty print
                        'utf8');
                    }
                }
            }
