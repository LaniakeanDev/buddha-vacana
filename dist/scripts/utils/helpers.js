...
function getSuttaData(nikaya, id, subnikaya) {
    ...
    '/public/_data/sutta', nikayaToSuttaPath, `/${String(id)}.json`);
...
const formatSuttaReference = (nikaya, suttaId, book) => `${nikaya != 'kn' ? nikaya.toUpperCase() : ''} ${book && nikaya != 'kn' ? `${book}.` : ''}${book && nikaya === 'kn' ? `${book} ` : ''}${suttaId}`;
function pageSuttaDataFetcher(nikaya, suttaId, book) {
    ...
    try {
        ...
        if (!suttaData)
            throw `File containing data for ${formatSuttaReference(nikaya, suttaId, String(book))} unavailable`;
